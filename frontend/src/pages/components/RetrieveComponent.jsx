...
import { Search, Download, Eye, Trash2, Share2, X, ImageOff } from "lucide-react";
...
const RetrieveComponent = () => {
  ...
  const handleView = async (doc) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_SERVER_API_URL}/api/file/retrieve`,
        {
          cid: doc.cid,
  ...
  const handleDelete = async (doc) => {
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_SERVER_API_URL}/api/file/delete`, {
        cid: doc.cid,
        fileName: doc.fileName,
