  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const signature = baseSignature
        ? baseSignature + (customName || selectedFile.name)
        : null;
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_SERVER_API_URL}/api/file/upload`,
        formData,
        {
          headers: {
      ...
      try {
        ...
        throw new Error(
          `Failed to store metadata on blockchain: ${error.message}`
        );
      }
    } catch (error) {
      ...
      <Card className="max-w-md mx-auto">
        ...
        <CardFooter>
          <Button
            className="w-full"
            disabled={
              !selectedFile || isUploading || solanaLoading || !solanaService
            }
