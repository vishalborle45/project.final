...
import type { documentStorage } from "../anchor/types/document_storage";
...
export class SolanaService {
  ...
  constructor(
    ...
    programIdString: string = "2aReMC6cg9RoeLrVtnXrpgaPKLo6cakpv7Ft77XQgR18"
  ) {
  ...
  async initializeUserDocuments(): Promise<string> {
    const [userDocumentsPDA] = await this.findUserDocumentsPDA(
  ...
  async addDocument(
    ...
    const [documentPDA] = await this.findDocumentPDA(
      this.wallet.publicKey,
      fileName
    );

    const tx = await this.program.methods
      .addDocument(fileName, cid, fileHash)
      .accounts({
        userDocuments: userDocumentsPDA,
  ...
  async getDocument(fileName: string): Promise<any> {
    ...
    try {
      const documentAccount = await this.program.account.document.fetch(
        documentPDA
      );
  ...
  async getUserDocumentStats(): Promise<any> {
    ...
    try {
      const userDocumentsAccount =
        await this.program.account.userDocuments.fetch(userDocumentsPDA);
